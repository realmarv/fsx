...
let thisScriptFileName = FileInfo(assemblyLocation).Name

if thisScriptFileName <> (sprintf "fsx.%s" assemblyExecutableExtension) then
    failwith(
        sprintf
            "this launcher should have been renamed to fsx.%s at install time; please report this bug"
            assemblyExecutableExtension
    )
